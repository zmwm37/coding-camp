...
# inspect data
summary(covid_data)
  # n_deaths 75th percentile is wayyy smaller than max! Outliers?


# inspect highest deaths quartile (>10k)
big_death <- covid_data %>%
  filter(n_deaths > 10000)
  # all values are from US aggregation
rm(big_death); gc()
...
# 6 - labs provides the title, and modifies x- and y-axis labels from default of variable name
us_deaths_by_race %>%
  ggplot(aes(y = race_ethnicity, x = percent_diff)) + 
  geom_col()


#7 reproduce #5 chart, only for Illinois
...
#10 - filter out data that is not mature
covid_data %>%
  filter(state == "US") %>%
  filter(race_ethnicity %in%
           c("Hispanic", "Non-Hispanic White", "Non-Hispanic Black", "Non-Hispanic Asian")) %>%
